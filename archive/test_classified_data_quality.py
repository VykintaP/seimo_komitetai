...
def test_classified_data_quality():
    ...
    for file in files:
        ...
        assert not df["predicted_topic_lt"].isna().all(), f"{file.name}: visos temos tuščios"
        assert (df["confidence_score"] > 0.3).any(), f"{file.name}: visos confidence < 0.3"
    ...
    assert top_ratio < 0.7, f"Dominuoja viena tema: '{top_counts.index[0]}' ({top_ratio:.2%})"
