...
# Spalvų gradientas stulpelinei diagramai nuo šviesios iki tamsios
LINE_CHART_GRADIENT = [
    "#FCFFDD",
    "#FAFDCB",
    "#F9EDB9",
...
def get_line_figure(
        df: pd.DataFrame, selected_topic: str = None, selected_date: str = None
) -> go.Figure:
    """Sukuria stulpelinę diagramą pagal klausimų kiekį per savaitę"""
    try:
        if (
                df is None
                or df.empty
                or "data" not in df.columns
                or "tema" not in df.columns
        ):
            return go.Figure()

        # Paruošiame datą analizei
        df["data"] = pd.to_datetime(df["data"], errors="coerce")
        df = df.dropna(subset=["data"])

        # Filtruojame duomenis pagal pasirinktą temą
        if selected_topic:
            df = df[df["tema"] == selected_topic]

        # Grupuojame įrašus pagal savaitės pradžią 
        df["savaite_nuo"] = df["data"] - pd.to_timedelta(
            df["data"].dt.weekday, unit="d"
        )
        df["savaite_nuo"] = df[
            "savaite_nuo"
        ].dt.date  # Pašalina laiką, palieka tik datą
        ...
        # Priskiriame spalvas pagal klausimų kiekį
        ...
        if min_val == max_val or max_val <= 3:
            yaxis_range = [0, max_val + 1]  # Pakelia dugną geresniam vaizdui
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                ...
                range=yaxis_range,  # Taikomas minimalus rėžis
            ),
        )
