...
# Funkcija komitetų sąrašui gauti
...
def update_filtered_df(start_date, end_date, selected_committee, selected_topic, selected_date):
    import re
    ...
    # Nustatome datų filtrą
    date_for_filtering = None
    if selected_date and isinstance(selected_date, str) and selected_date.strip() and selected_date.strip() != "RESET":
        ...
        if date_match:
            ...
            logging.warning("[filtered-df] Nepavyko išskirti datos iš selected-date: %s", selected_date)
...
def filter_bar_chart_data(raw_data, selected_topic, selected_date):
    ...
    if selected_date and isinstance(selected_date, str) and selected_date.strip() != "RESET":
        date_match = re.match(r"(\d{4}-\d{2}-\d{2})", selected_date.strip())
        if date_match:
